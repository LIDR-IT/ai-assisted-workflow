# References

Detailed technical documentation about systems, APIs, and architecture.

## 🤖 Agents

Agent system documentation and platform-specific guides:

### Standards & Formats

- **[Agent Format Standard](./agents/AGENT_FORMAT_STANDARD.md)** - Complete agent file format specification
- **[Minimal Format Standard](./agents/MINIMAL_FORMAT_STANDARD.md)** - Minimal viable agent format
- **[Platform Comparison](./agents/PLATFORM_COMPARISON.md)** - Agent capabilities across platforms
- **[Agents.md Format](./agents/agents-md-format.md)** - AGENTS.md file format

### Platform-Specific

- **[Claude Code Agent Development](./agents/agent-development-claude-code.md)** - Creating agents for Claude Code
- **[Claude Code Sub-Agents](./agents/sub-agents-claude-code.md)** - Sub-agents in Claude Code
- **[Cursor Subagents](./agents/cursor-subagents.md)** - Subagents in Cursor
- **[Gemini Subagents](./agents/subagents-gemini.md)** - Subagents in Gemini CLI

## 🔌 MCP (Model Context Protocol)

- **[MCP Overview](./mcp.md)** - General MCP introduction
- **[MCP Introduction](./mcp/mcp-introduction.md)** - Getting started with MCP
- **[MCP Server Builder](./mcp/mcp-server-builder.md)** - Building custom MCP servers

### Platform Integration

- **[Claude Code MCP Usage](./mcp/mcp-usage-claude-code.md)** - Using MCP in Claude Code
- **[Claude Code MCP Integration](./mcp/mcp-integration-claude-code.md)** - Integrating MCP servers
- **[Cursor MCP](./mcp/mcp-cursor.md)** - MCP in Cursor
- **[Gemini CLI MCP](./mcp/mcp-gemini-cli.md)** - MCP in Gemini CLI
- **[Antigravity MCP](./mcp/mcp-antigravity.md)** - MCP in Antigravity

## 🎯 Skills

Skills ecosystem and development:

- **[Skills Overview](./skills.md)** - General skills introduction
- **[Skills Ecosystem Overview](./skills/skills-ecosystem-overview.md)** - Complete ecosystem overview
- **[Skills README](./skills/README.md)** - Skills documentation index

### Development & Tools

- **[Claude Code Skill Development](./skills/skill-development-claude-code.md)** - Creating skills for Claude Code
- **[Claude Code Skills](./skills/skills-claude-code.md)** - Using skills in Claude Code
- **[Skill Creator](./skills/skill-creator.md)** - Skill creation guide
- **[npm Skills Package](./skills/npm-skills-package.md)** - Publishing skills as npm packages

### Platform-Specific

- **[Antigravity Skills](./skills/antigravity-skills.md)** - Skills in Antigravity
- **[OpenSkills](./skills/openskills.md)** - Open source skills library
- **[Find Skills (Vercel)](./skills/find-skills-vercel.md)** - Finding and installing skills

## 💬 Commands

Custom commands and slash commands:

- **[Command Development](./commands/command-development.md)** - Creating custom commands
- **[Cursor Commands](./commands/cursor-commands.md)** - Commands in Cursor
- **[Gemini CLI Custom Commands](./commands/gemini-cli-custom-commands.md)** - Commands in Gemini CLI
- **[Antigravity Workflows](./commands/antigravity-workflows.md)** - Workflows in Antigravity

## 🪝 Hooks

Git hooks and automation:

- **[Hooks Reference](./hooks/hooks-reference.md)** - Complete hooks reference
- **[Automate Workflows with Hooks](./hooks/automate-workflows-with-hooks.md)** - Workflow automation
- **[Claude Code Hooks Guide](./hooks/hooks-guide-claude-code.md)** - Hooks in Claude Code
- **[Gemini CLI Hooks](./hooks/gemini-cli-hooks.md)** - Hooks in Gemini CLI

## 📋 Rules

Project rules and memory management:

- **[Rules README](./rules/README.md)** - Rules system overview
- **[Memory and Rules](./rules/memory-and-rules.md)** - How rules and memory work

### Platform-Specific

- **[Cursor Rules](./rules/cursor-rules.md)** - Rules in Cursor
- **[Gemini.md](./rules/gemini-md.md)** - Gemini CLI memory system
- **[Antigravity Rules](./rules/antigravity-rules.md)** - Rules in Antigravity
- **[Antigravity Rules & Workflows](./rules/antigravity-rules-workflows.md)** - Complete Antigravity setup

## 📝 Planning & Tasks

Task planning and memory systems:

- **[Setup Planning](./planning-tasks/SETUP_PLANNING.md)** - Planning setup guide
- **[Cursor Plan Mode](./planning-tasks/cursor-plan-mode.md)** - Using Cursor's plan mode
- **[Gemini MD Hierarchy](./planning-tasks/gemini-md-hierarchy.md)** - Gemini memory hierarchy
- **[Gemini Memory](./planning-tasks/gemini-memory.md)** - Gemini memory system

## 🔧 Claude Code

Claude Code CLI specific documentation:

- **[CLI Reference](./claude-code/cli-reference.md)** - Complete CLI command reference
- **[Discover Plugins](./claude-code/discover-plugins.md)** - Finding and installing plugins
- **[Analytics](./claude-code/analytics.md)** - Usage analytics
- **[Security](./claude-code/security.md)** - Security considerations

## 📐 Documentation

Documentation tools and standards:

- **[C4 Architecture](./documentation/c4-architecture.md)** - C4 model for architecture diagrams
- **[Mermaid Diagrams](./documentation/mermaid-diagrams.md)** - Creating diagrams with Mermaid

## 📚 Additional Resources

For practical guides, see:

- [Guides](/en/guides/) - Step-by-step tutorials
- [Modules](/en/modules/skills/) - In-depth learning modules
- [Guidelines](/en/guidelines/) - Team standards and conventions

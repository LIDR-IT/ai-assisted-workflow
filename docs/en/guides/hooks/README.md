...
2. **[sync.sh](.agents/sync.sh)** (unified CLI)
   ...
   - Run with `--only=hooks` for hooks-only sync
...
**Don't edit these directly - generated by `.agents/sync.sh --only=hooks`**
...
./.agents/sync.sh --only=hooks --dry-run
...
- Created cursor conversor in sync.sh
...
3. Check sync: `./.agents/sync.sh --only=hooks --dry-run`
...
A: Run `./.agents/sync.sh --only=hooks` to regenerate platform configs. Changes in hooks.json require sync.

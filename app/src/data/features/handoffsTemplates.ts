...
export const handoffs: HandoffResponsibility[] = [
  ...
  },
  {
    ...
    aiAutomation:
      'BMad produce: bmad-create-architecture (⭐G2) + bmad-create-epics-and-stories (⭐G2) + bmad-testarch-test-design. LIDR gobierna: lidr-requirements (per-rf → nfr → validate modes; el modo validate genera la RTM, ⭐G2); /lidr-validate-requirements orquesta la fase completa',
    color: 'bg-cyan-50',
...
export const phases: PhaseTemplates[] = [
    ...
    templates: [
      {
        ...
        desc: 'PRD con secciones Funcional + Técnica en un solo documento, generado por bmad-prd desde el product-brief + business-case. Evidencia REQUIRED del G1 (_bmad-output/planning-artifacts/prds/**/prd.md). Input de bmad-ux, bmad-create-architecture y lidr-requirements (per-rf mode).',
        format: '_bmad-output/planning-artifacts/',
        owner: 'PO + R&D',
        mandatory: true,
        aiAssist: 'skill',
      ...
      },
      {
        ...
        mandatory: false,
        aiAssist: 'skill',
        claudePath: '.claude/skills/lidr-review-cruzado/SKILL.md',
      },
      {
        code: 'T-PRD-004',
        name: 'Risk Log',
  ...
  },
  {
    ...
    exitCriteria: [
      ...
      'Cada RF tiene criterios de aceptación BDD Given/When/Then (lidr-requirements per-rf mode)',
      'RTM mapea RF ↔ epic/story ↔ test sin requisitos huérfanos (lidr-requirements validate mode)',
      'Epics descompuestas en stories de 2-40h (bmad-create-epics-and-stories)',
      'NFRs standalone con métrica medible y umbral (lidr-requirements nfr mode)',
      'Bloque TEA arranque (si el equipo adopta TEA): test-design → framework → ci, en orden estricto — opcional',
    ],
    ...
    templates: [
      ...
      },
      {
        ...
        desc: 'Descomposición de la épica master en epics y stories desde PRD + architecture (bmad-create-epics-and-stories). Evidencia REQUIRED del G2; el checklist exige stories de 2-40h. PO y TL validan agrupaciones. Produce el ESQUELETO épica→story (el artefacto de stories que el gate verifica); lidr-user-stories (G3, T-SPR-001) lo refina en US RF-trazables con BDD+INVEST. No son dos backlogs paralelos: son dos profundidades del mismo (esqueleto BMad → US implementable LIDR).',
        format: '_bmad-output/planning-artifacts/',
        owner: 'PO + TL',
        mandatory: true,
        aiAssist: 'skill',
      ...
      },
      {
        ...
        mandatory: true,
        aiAssist: 'skill',
        claudePath: '.claude/skills/lidr-requirements/SKILL.md',
      },
      {
        code: 'T-RF-004',
        name: 'RTM (Requirements Traceability Matrix)',
        desc: 'Trazabilidad completa US → RF/NFR → PRD → Business Case sin huérfanos, generada por lidr-requirements (validate mode). Evidencia REQUIRED del G2 (rtm.md); también evidencia de trazabilidad en G5. /lidr-validate-requirements orquesta la fase completa.',
      ...
      },
      {
        ...
        desc: 'Estrategia de testing risk-based diseñada en Solutioning desde epics + NFRs (bmad-testarch-test-design, módulo TEA). Primer paso del bloque TEA arranque, orden estricto test-design → framework → ci (BMad 6.8.0: las tres son 3-solutioning). Evidencia opcional del G2 (_bmad-output/test-artifacts/test-design/). Input de lidr-create-test-cases y bmad-testarch-automate en la fase qa.',
      ...
      {
        code: 'T-SOL-003',
        name: 'Test Framework (BMad TEA)',
        desc: 'Inicializa el framework de testing (Playwright o Cypress) con buenas prácticas, implementando el plan de test-design. Segundo paso del bloque TEA arranque (test-design → framework → ci); el framework debe quedar instalado antes del primer ATDD en development. Evidencia opcional del G2 (_bmad-output/test-artifacts/).',
        format: '_bmad-output/test-artifacts/',
        owner: 'QA Lead',
        mandatory: false,
        aiAssist: 'skill',
        claudePath: '.claude/skills/bmad-testarch-framework/SKILL.md',
      },
      {
        code: 'T-SOL-005',
        name: 'CI Quality Pipeline (BMad TEA)',
        desc: 'Configura la pipeline de integración continua con quality gates de testing sobre el framework instalado (bmad-testarch-ci). Tercer y último paso del bloque TEA arranque (test-design → framework → ci); debe estar lista antes del primer merge. Evidencia opcional del G2 (_bmad-output/test-artifacts/).',
        format: '_bmad-output/test-artifacts/',
        owner: 'QA + DevOps',
        mandatory: false,
        aiAssist: 'skill',
        claudePath: '.claude/skills/bmad-testarch-ci/SKILL.md',
      },
      {
        code: 'T-SOL-004',
  ...
  },
  {
    ...
    templates: [
      ...
      },
      {
        ...
        desc: 'Capacidad por miembro: días disponibles, % dedicación, horas efectivas, buffer 15-20% (lidr-sprint-capacity, usa el risk-log para dimensionar buffer). Evidencia REQUIRED del G3 (sprint-capacity*.md). El checklist exige commitment ≤90% de la capacidad neta. Es un INPUT de bmad-sprint-planning: aporta la cifra de capacidad comprometible que sprint-planning consume al generar el sprint-status.yaml (productor → consumidor, no solapan).',
        format: 'docs/projects/{client}/',
        owner: 'Scrum Master',
        mandatory: true,
        aiAssist: 'skill',
      ...
      {
        code: 'T-SPR-001',
        name: 'User Stories (BDD + INVEST)',
        desc: 'Backlog de US sprint-ready: slicea los RFs aprobados (G2) con 8 patrones de slicing, hereda el BDD del RF y valida INVEST (lidr-user-stories). REFINA el esqueleto épica→story de bmad-create-epics-and-stories (T-RF-005): BMad da la estructura/gate artifact, LIDR produce las US RF-trazables e implementables que son el backlog comprometido que consume bmad-sprint-planning. Evidencia opcional del G3 (no bloquea: el artefacto de gate de stories es el de BMad en G2).',
        format: 'docs/projects/{client}/',
        owner: 'PO',
        mandatory: false,
        aiAssist: 'skill',
      ...
      {
        code: 'T-SPR-002',
        name: 'Refinement Notes + DoR',
        desc: 'Formaliza el refinement y el checklist Definition of Ready por story (lidr-refinement-notes). El DoR cumplido es el item de checklist bloqueante del G3 ("Definition of Ready met for every committed story"), así que aunque el doc sea evidencia opcional, su contenido alimenta el gate. Evidencia opcional del G3.',
        format: 'docs/projects/{client}/',
        owner: 'Scrum Master',
        mandatory: false,
        aiAssist: 'skill',
  ...
  },
  {
    ...
    templates: [
      ...
      },
      {
        ...
        mandatory: true,
        aiAssist: 'command',
        claudePath: '.claude/commands/lidr-advance-gate.md',
      },
      {
        code: 'T-DEV-003',
      ...
      {
        code: 'T-DEV-009',
        name: 'ATDD Red-Phase (BMad TEA)',
        desc: 'Acceptance tests en fase roja por cada story, generados ANTES de implementar (bmad-testarch-atdd, módulo TEA). Primer paso de la cadena per-story de orden estricto atdd → dev-story → automate → code-review; requiere el framework instalado en G2. Evidencia opcional del G4 (_bmad-output/test-artifacts/).',
        format: '_bmad-output/test-artifacts/',
        owner: 'QA + Dev',
        mandatory: false,
        aiAssist: 'skill',
        claudePath: '.claude/skills/bmad-testarch-atdd/SKILL.md',
      },
    ],
  },
  {
    ...
    templates: [
      ...
      },
      {
        ...
        desc: 'Matriz de trazabilidad RTM ↔ resultados + recomendación de gate decision (bmad-testarch-trace). Cierre de épica: la cadena estricta es test-review → trace (trace audita la cobertura sobre la suite ya revisada). Evidencia opcional del G5 (test-artifacts/traceability/).',
        format: '_bmad-output/test-artifacts/',
        owner: 'QA Lead',
        mandatory: false,
        aiAssist: 'skill',
      ...
      {
        code: 'T-QA-008',
        name: 'Test Quality Review (BMad TEA)',
        desc: 'Revisión de la calidad de los tests acumulados al cierre de épica con validación de buenas prácticas, devuelve puntuación 0-100 por dimensión (bmad-testarch-test-review). Precede a trace en la cadena de cierre de épica (test-review → trace). Evidencia opcional del G5 (_bmad-output/test-artifacts/).',
        format: '_bmad-output/test-artifacts/',
        owner: 'QA Lead',
        mandatory: false,
        aiAssist: 'skill',
        claudePath: '.claude/skills/bmad-testarch-test-review/SKILL.md',
      },
      {
        code: 'T-QA-009',
        name: 'NFR Audit (BMad TEA)',
        desc: 'Auditoría de la evidencia NFR (performance, security, reliability, scalability) post-implementación contra los NFRs medibles del G2 (bmad-testarch-nfr). En BMad 6.8.0 es un skill único de phase-4 implementation — no una secuencia de 3 momentos. Evidencia opcional del G5 (_bmad-output/test-artifacts/).',
        format: '_bmad-output/test-artifacts/',
        owner: 'QA + DevOps',
        mandatory: false,
        aiAssist: 'skill',
        claudePath: '.claude/skills/bmad-testarch-nfr/SKILL.md',
      },
    ],
  },
  {

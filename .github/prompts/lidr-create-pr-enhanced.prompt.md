---
description: Enhanced PR creation with SDLC tracking and automated handoffs
agent: 'agent'
---

<!--
COMMAND: create-pr-enhanced
VERSION: 2.0.0
...
USAGE:
  /create-pr-enhanced PROJ-123
  /create-pr-enhanced PROJ-123 --handoff=auto
  /create-pr-enhanced PROJ-123 --draft --security-review
  /create-pr-enhanced PROJ-123 --project=PROJ-2026-001
...
RELATED COMMANDS:
  /create-branch-enhanced - Enhanced branch creation
...
If "$1" is empty:
  ❌ Usage: /create-pr-enhanced [TICKET-ID] [flags]

  Examples:
    /create-pr-enhanced PROJ-123
    /create-pr-enhanced PROJ-123 --handoff=auto --security-review
    /create-pr-enhanced PROJ-123 --draft --project=PROJ-2026-001
  Exit.
...
Current branch validation: Ensure follows enhanced naming convention from create-branch-enhanced.
...
If no commits ahead:
  ...
  Suggestions:
    1. Make code changes for story $1
    2. Commit with conventional format: "feat(PROJ-123): description"
    3. Run /create-pr-enhanced $1 again
  Exit.
...
- **External Tools**:
  - Jira: [$1]({jira-link})
  - Linear: [{linear.id}]({linear-link}) *(if applicable)*
  - Notion: [{notion.id}]({notion-link}) *(if applicable)*
...
🛠️ Recovery Options:
   1. Auto-fix:    /create-pr-enhanced $1 --fix-{specific.issue}
   2. Manual fix:  {specific.manual.steps}
   3. Skip sync:   /create-pr-enhanced $1 --sync=false
...
- `/create-pr-enhanced` provides all new functionality

...
test cases (BDD), regression candidates, and exports CSV for {{TEST_MGMT_TOOL}} import.

USAGE:
  /lidr-prepare-testing PROJ-123

ARGUMENTS:
  ticket-id: Tracking-tool ticket ID (required)
...
REQUIREMENTS:
  ...
  - CSV export for {{TEST_MGMT_TOOL}} integration
...
From the tracking ticket (via lidr-sdlc-tracking, resolves {{TRACKING_TOOL}}):
...
Generate CSV export for {{TEST_MGMT_TOOL}} import (via lidr-external-sync, which resolves {{TEST_MGMT_TOOL}} from the registry):

- Create test plan and test cases in CSV format
- Save to `.claude/testing/$1-test-suite.csv`
- Ready for {{TEST_MGMT_TOOL}} import via lidr-external-sync
- Also generate markdown document for review
- Save to `.claude/testing/$1-test-suite.local.md`

## Transition

Via lidr-sdlc-tracking (resolves {{TRACKING_TOOL}}): add comment to $1 with test plan summary
Via lidr-sdlc-tracking (resolves {{TRACKING_TOOL}}): transition $1 to "In Testing"

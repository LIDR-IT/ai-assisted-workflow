...
framework, so the built-in domain pattern set (used by the optional
domain_compliance step) is empty: out of the box validation relies only on
generic checks (structure, density, measurability, traceability, completeness,
etc.). To enable domain-coverage checks for a specific industry, pass a config
file (--config) with a ``domain_patterns`` mapping, or merge the example pack.

An overridable EXAMPLE industry pack (biometric identity) is preserved below as
the BIOMETRIC_EXAMPLE_DOMAIN_PATTERNS constant and as a sibling file
`validation-engine.biometric-example.json` (same schema as the ``domain_patterns``
config key). It is an example only — NOT the active default.
"""
...
#
# This is an EXAMPLE of an industry override for the ``domain_patterns`` config
# key, NOT the active default. The default config (see ValidationEngine._load_config)
# ships with an EMPTY ``domain_patterns`` map so the domain_compliance step is a
# no-op unless a project opts in. The same content is shipped as the sibling JSON
# file `validation-engine.biometric-example.json`; merge it into a --config file
# (under the ``domain_patterns`` key) to enable biometric domain-coverage checks.
# ---------------------------------------------------------------------------
BIOMETRIC_EXAMPLE_DOMAIN_PATTERNS = {
    "biometric_identity": ["GDPR", "Article 9", "biometric", "template", "FAR", "FRR", "EER"],
    "document_verification": ["KYC", "onboarding", "OCR", "passport", "DNI", "id document"],
    "healthcare": ["HIPAA", "PHI", "diagnosis", "clinical", "health"],
}
...
class ValidationEngine:
    ...
    def _load_config(self, config_path: Optional[str]) -> Dict:
        ...
        default_config = {
            ...
            # DOMAIN-AGNOSTIC default: no industry patterns. The domain_compliance
            # step is a no-op (and _detect_domain returns "generic") unless a
            # project opts in by providing a `domain_patterns` mapping via --config.
            # See BIOMETRIC_EXAMPLE_DOMAIN_PATTERNS (and the sibling
            # validation-engine.biometric-example.json) for an opt-in example.
            "domain_patterns": {}
        }
...
**Generated by the LIDR SDLC Validation Engine v1.0**
"""

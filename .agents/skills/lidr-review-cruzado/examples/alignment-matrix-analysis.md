...
**PRD scopes**: one prd.md — Functional v2.1 ↔ Technical v2.0
...
| Functional Functionality         | Technical Capability           | Status        | Analysis                                                                  |
| -------------------------------- | ------------------------------ | ------------- | ------------------------------------------------------------------------- |
| **Facial Recognition** (§4.1)    | **Face Algorithm v3.2** (§3.1) | ✅ ALIGNED    | Both specify same algorithm version, performance targets match            |
| **Liveness Detection** (§4.1)    | **3D Liveness Module** (§3.2)  | ⚠️ PARTIAL    | Functional: passive detection; Technical: active + passive (clarify UX)   |
| **Voice Authentication** (§4.2)  | **Voice Engine v2.1** (§3.4)   | ❌ MISALIGNED | Functional: 99.5% accuracy; Technical: 97.8% lab results (**CF-004**)     |
| **Document Verification** (§4.3) | **OCR Engine + NFC** (§3.5)    | ✅ ALIGNED    | Full feature parity, security requirements match                          |
| **Behavioral biometrics** (§4.4) | **Typing Dynamics** (§3.6)     | ⚠️ PARTIAL    | Functional: full behavioral; Technical: typing only (scope clarification) |
| **Multi-modal Fusion** (§4.5)    | ⭕ MISSING                     | ❌ MISALIGNED | Functional specifies combined modalities; no technical implementation     |

---

## 2. User Journey ↔ API Flow Mapping

| Functional User Journey Step  | Technical API Endpoint         | Status        | Technical Mapping                                                   |
| ----------------------------- | ------------------------------ | ------------- | ------------------------------------------------------------------- |
...
| **2.5 Result Communication**  | `WebSocket /v3/results/stream` | ⚠️ PARTIAL    | Functional: instant; Technical: near real-time (0.5s)               |
| **2.6 Error Recovery**        | Multiple error endpoints       | ❌ MISALIGNED | Functional: seamless retry; Technical: manual intervention required |

---

## 3. GDPR Compliance ↔ Technical Implementation

| Functional GDPR Requirement    | Technical Technical Solution           | Status        | Compliance Gap                                               |
| ------------------------------ | -------------------------------------- | ------------- | ------------------------------------------------------------ |
...
| **Data Portability** (§6.4)    | ⭕ MISSING                             | ❌ MISALIGNED | Functional requires export; no technical implementation      |
...
| Functional Performance Target | Technical System Capability  | Status        | Gap Analysis                                     |
| ----------------------------- | ---------------------------- | ------------- | ------------------------------------------------ |
| **Verification Speed**        |                              |               |                                                  |
...
| Functional Error Scenario    | Technical Failover Mechanism     | Status     | Robustness Assessment                          |
| ---------------------------- | -------------------------------- | ---------- | ---------------------------------------------- |
...
| Functional Security Requirement | Technical Security Implementation | Status     | Security Posture                    |
| ------------------------------- | --------------------------------- | ---------- | ----------------------------------- |
| **Authentication**              |                                   |            |                                     |
...
1. **Multi-modal Fusion Missing** (Functional §4.5 → Technical gap)
   - **Impact**: Core functionality not technically specified
   - **Action**: Add multi-modal processing architecture to Technical

2. **Voice Authentication Accuracy Gap** (Functional 99.5% → Technical 97.8%)
   - **Impact**: May not meet business requirements
   - **Action**: Algorithm evaluation or requirement adjustment

3. **GDPR Right to Erasure** (Functional complete deletion → Technical partial)
   - **Impact**: Regulatory compliance violation
   - **Action**: Implement ML model retraining for erasure

4. **Performance Gap - Mobile** (Functional <2s → Technical 2.8-3.2s)
   - **Impact**: User experience degradation
   - **Action**: Algorithm optimization or hardware requirements

### Development Phase Resolution

5. **Data Portability Implementation** (Functional requirement → Technical missing)
6. **Error Recovery Automation** (Functional seamless → Technical manual)
7. **Breach Notification Automation** (Functional 72h → Technical manual process)

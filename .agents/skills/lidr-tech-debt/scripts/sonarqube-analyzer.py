...
class SonarQubeAnalyzer:
    ...
    def fetch_issues_from_api(self, pageSize: int = 500) -> List[Dict]:
        ...
        if not self.token:
            print("❌ SonarQube token not provided. Use --token or set environment variable SONAR_TOKEN")
            return []
    ...
    def analyze_issues(self) -> Dict[str, TechnicalDebtItem]:
        ...
        print(f"🔍 Analyzing {len(self.raw_issues)} SonarQube issues...")
    ...
    def _generate_evidence(self, issues: List[Dict], most_common_rule: str) -> str:
        ...
        evidence = f"**SonarQube Analysis Results:**\n"
...
updated_by: "Sistema: SonarQube Analyzer"
status: active
type: registry
...
| **SonarQube Issues** | {item.sonarqube_issues_count} |
| **Rules** | {rules_display} |
...
### Severity Mapping
- **Critical**: CRITICAL SonarQube issues (blocking)
- **High**: MAJOR SonarQube issues (significant)
- **Medium**: MINOR SonarQube issues (moderate)
- **Low**: INFO SonarQube issues (informational)
...
*Generated by {{CLIENT_NAME}} SonarQube Technical Debt Analyzer*
...
def main():
    parser = argparse.ArgumentParser(description="{{CLIENT_NAME}} SonarQube Technical Debt Analyzer")
    ...
    print("🚀 {{CLIENT_NAME}} SonarQube Technical Debt Analyzer")
    print("=" * 55)
    ...
    # Summary
    print(f"\n🎯 Analysis Summary:")
    print(f"   SonarQube Issues: {len(analyzer.raw_issues)}")

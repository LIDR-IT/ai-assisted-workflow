...
mcps:
  - filesystem
evolvedFrom: security skills (lidr-vuln-assessment + lidr-dast-interpretation + lidr-security-checklist)
---
...
<commentary>
Pre-deploy security evaluation. Agent runs lidr-security-checklist skill against codebase and config.
</commentary>
</example>
...
1. **GUARD: Verify prerequisites before execution**
   ...
   - Verify Gate 4 (Dev -> QA) has passed if running pre-Gate 6 evaluation — read `docs/projects/{client}/handoffs/gate-4-handoff.md`
...
3. Interpreta hallazgos con skill lidr-vuln-assessment o lidr-dast-interpretation
...
7. Evalua lidr-security-checklist pre-deploy
...
4. **Prioritize**: Using preloaded lidr-vuln-assessment skill:
...
7. **Evaluate Checklist**: Using preloaded lidr-security-checklist skill, run full pre-deploy check

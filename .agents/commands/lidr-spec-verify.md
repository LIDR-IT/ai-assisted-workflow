...
drift via /lidr-sync-docs principles, and produce a final test-report.md with PASS /
...
- `status` is `active` after a PASSED finalize (see "Finalize artifact status" below); `in-progress`/`draft` before verification
- If broken: WARNING
...
## Finalize artifact status (on PASSED)

If the verdict is **PASSED**, transition the change artifacts to their applied state — this is the **only** step in the lifecycle that does so (`/lidr-spec-new` scaffolds `draft`, `/lidr-spec-ff` sets `in-progress`):

- Set `status: active` on `proposal.md`, `design.md`, `spec.md`, `tasks.md`
- Append an `editHistory` entry (step: `spec-verify`, "Verified PASSED — status set to active")

Without this, `status` would stay `in-progress` forever and Check 7 would warn on every run.
...
Next:
  PASSED      → test-report.md verdict is the Gate 4 (Dev→QA) evidence → /lidr-advance-gate 4, then /lidr-spec-archive <change-name>

...
updated_by: "TL: SDD single-sequence coherence"
status: active
tier: 2
authorized_roles:
  - PO
...
Rapidly generate lightweight requirements specification for small features (≤ 40h effort) without full PRD overhead. Based on the LIDR SDLC Methodology's quick specification pattern — **domain-agnostic** (any domain-specific compliance is bound per client config via `{{...}}` variables, never hardcoded).

> **Relationship to BMad — one sequence, no parallel route.** `/lidr-quick-spec` is the lightweight intent **front-end**; the BMad engine for small changes is `bmad-quick-dev` (intent → hardened artifact) / `bmad-spec` (SPEC kernel). quick-spec does not compete with them — its output feeds the same `/lidr-spec-*` governance chain, which delegates implementation to the BMad engine (`bmad-dev-story`). See `lidr-sdlc/spec-execution.md` §0.

## When to Use

### ✅ Use `/lidr-quick-spec` for:
...
- Architectural changes (use `lidr-adr` skill)
...
```bash
/lidr-quick-spec [feature-name] [--type=enhancement|bugfix|integration|api] [--effort=hours]
```

### Examples

```bash
/lidr-quick-spec user-avatar-upload --type=enhancement --effort=24
/lidr-quick-spec session-timeout-fix --type=bugfix --effort=8
/lidr-quick-spec webhook-notifications --type=integration --effort=32
```
...
1. **Security / abuse vectors**: Test against the domain's relevant attack and abuse cases
2. **Performance**: Response time under load
3. **Accuracy / correctness**: Validate the domain's quality metrics
...
- [ ] Data-protection compliance relevant to the data handled (e.g. GDPR)
- [ ] Domain / industry regulatory requirements (bound per client config)
...
1. **bmad-document-project** (if needed to understand project context and find related existing specs)

### Called Skills During Execution
1. **lidr-requirements** (per-rf mode, simplified - max 8 RFs)
2. **lidr-user-stories** (single story generation)
3. **lidr-requirements** (validate mode, lightweight validation)

### Post-execution Skills
1. **lidr-create-test-cases** (from generated RFs)
2. **bmad-create-epics-and-stories** (if feature needs sub-tasks)
...
- Features affecting core domain algorithms
...
- **Pre**: `/lidr-validate-project-docs` to ensure project context is current
- **Post**: feeds the single development sequence — paste the quick-spec doc as the enriched intent into `/lidr-spec-new` → `/lidr-spec-ff` → `/lidr-spec-apply` → `/lidr-spec-verify` → `/lidr-spec-archive`
- **Chain**: `/lidr-quick-spec` → `/lidr-spec-new` → ff → apply → verify (quick-spec is the lightweight planning front-end of the same chain, not a separate route)

### With Skills

- Uses lightweight versions of `lidr-requirements` (per-rf mode) and `lidr-user-stories`
- Can trigger `lidr-requirements` (validate mode) for consistency check
- Integrates with `lidr-create-test-cases` for automated test generation

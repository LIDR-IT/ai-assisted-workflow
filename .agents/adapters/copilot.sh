...
_copilot_generate_instructions_index() {
  local out="$COPILOT_DIR/copilot-instructions.md"
  local tmp="${out}.tmp"
...
EOF
    ...
    while IFS= read -r rule_file; do
      ...
      if [ "$subdir" != "$prev_subdir" ]; then
        [ -n "$prev_subdir" ] && echo ""
        printf "### %s\n\n" "$(_human_subdir_copilot "$subdir")"
        prev_subdir="$subdir"
      fi

      local flat_base
      flat_base=$(flat_rule_basename "$rule_file" "$rules_root")
      local desc
...
EOF
  ...
  _write_if_changed_copilot "$tmp" "$out" "copilot-instructions.md"
}

_human_subdir_copilot() {
  local s=$1
  case "$s" in
    "(root)")     echo "General" ;;
...
copilot_mcp() {
  ...
  # Source convention is POSIX ${VAR}; convert to ${env:VAR} for VSCode here.
  jq '{
    servers: (
      .servers |
      to_entries |
            ...
            if .type == "http" then
            ...
            else
              {
                ...
                env: (
                  ...
                  map({
                    key: .key,
                    value: (.value | gsub("\\$\\{(?<v>[A-Z_][A-Z0-9_]*)\\}"; "${env:\(.v)}"))
                  }) |
                  from_entries
                )
              }
  ...
  }' "$AGENTS_DIR/mcp/mcp-servers.json" | write_if_changed "$COPILOT_MCP_DIR/mcp.json" ".vscode/mcp.json"
}
...
copilot_hooks() {
  ...
  "hooks": {}
    ...
    # VSCode/Copilot schema (per docs):
    #   - PascalCase event names (PreToolUse/PostToolUse) — both surfaces accept these
    #   - REQUIRED field "type": "command" per VSCode docs
    #   - matcher is parsed but NOT enforced by VSCode (kept for Copilot CLI compatibility)
  ...
  echo "$copilot_hooks_json" | write_if_changed "$COPILOT_DIR/hooks/hooks.json" ".github/hooks/hooks.json ($hook_count hook types)"
}
...
copilot_verify() {
  ...
  verify_link_or_copy "$COPILOT_DIR/hooks/scripts" "copilot hooks scripts" || ((errors++))

...
| **3 / specification**   | `bmad-create-architecture` ⭐G2, `bmad-create-epics-and-stories` ⭐G2, `bmad-testarch-test-design`, `bmad-testarch-framework`, `bmad-testarch-ci`    | `lidr-requirements` (**per-rf · nfr · validate** modes; RTM — reads the ONE unified PRD's functional + NFR scope; its `living-spec`/`brownfield-audit` modes run in Phase 0) ⭐G2, `lidr-adr`                                                                                                                                                                                                                                              |
...
PHASE 3 — SOLUTIONING / specification
  ...
  lidr-requirements per-rf (PRD unificado: alcance funcional) → RF-*.md con BDD ⭐G2  [→ user-stories, test-cases, RTM]
  lidr-requirements nfr (PRD unificado: alcance NFR) → NFR-*.md medibles              [→ testarch-nfr, security]
...
PHASE 0 — CONTEXT  (OBLIGATORIA en brownfield, antes de cualquier análisis)
  ...
  lidr-requirements brownfield-audit (bmad-document-project output) → docs/features/<f>/spec.md recuperado  [→ bmad-testarch-trace coverage audit]
  ...
  ▸ Checklist "Context Ready" (evidencia brownfield de G0):
    ...
    [ ] spec de features críticas recuperada (lidr-requirements brownfield-audit si aplica)
...
| ------- | ---------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.4.0   | 2026-06-14 | TL: unified-PRD + multi-phase     | `lidr-requirements` es multi-fase: §2 reubica sus modos `living-spec` (0/anytime) y `brownfield-audit` (0/context) a Phase 0; `per-rf·nfr·validate` siguen en 3/specification (G2). Coherencia con BMad: el PRD es UNO (FR+NFR en un doc) — §3.1/§3.2 dejan de citar "PRD-F/PRD-T §5". §3.2 brownfield añade brownfield-audit en Phase 0 |
